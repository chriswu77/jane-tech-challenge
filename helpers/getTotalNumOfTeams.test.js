const getTotalNumOfTeams = require('./getTotalNumOfTeams')

describe('getTotalNumOfTeams', () => {
    test('returns 6 (count of unique teams) from array of string results', () => {
        ...
        expect(getTotalNumOfTeams(exampleResults)).toBe(6)
    })

    test('returns 4 (count of unique teams) from array of string results', () => {
        const exampleResults = [
            'Capitola Seahorses 5, San Jose Earthquakes 5',
            'Santa Cruz Slugs 1, Felton Lumberjacks 1'
        ]

        expect(getTotalNumOfTeams(exampleResults)).toBe(4)
    })
})
